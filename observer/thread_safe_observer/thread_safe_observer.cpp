	...
	void field_changed(Person& source, const std::string& fieldname) override {
		if (fieldname == "age") {
			...
			if (source.get_age() >= 18) {
				std::cout << "You can drive now. We are no longer interested in tracking your age" << std::endl;
				source.unsubscribe(*this); // tries to lock the std::scoped_lock<std::mutex> within the Observable implementation and this will result in a deadlock
			}
		}
